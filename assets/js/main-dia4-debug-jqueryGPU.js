...
$(document).ready(async function () {
  ...
  const imageCount = 84;
  const cdnBase = 'https://frgcdn-dia4-sd.xperia.pt';
  const cdnHD = 'https://frgcdn-dia4-sd.xperia.pt';
    ...
    try {
      ...
      if (err instanceof TypeError) {
        try {
          ...
          return fallbackRes.type === 'opaque';
        } catch (fallbackErr) {
  ...
  const loadImageTasks = Array.from({ length: imageCount }, (_, idx) => (async () => {
    ...
    const $button = $(`
      <button class="gallery-download-btn" aria-label="Download HD">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
          ...
          <path d="M7.646 10.854a.5.5 0 0 0 .708 0l3.5-3.5a.5.5 0 0 0-.708-.708L8.5 9.293V1.5a.5.5 0 0 0-1 0v7.793L5.354 6.646a.5.5 0 1 0-.708.708l3.5 3.5z"/>
        </svg> 
      </button>
    `);
