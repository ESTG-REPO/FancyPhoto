document.addEventListener('DOMContentLoaded', async function () {
  ...
  function createGalleryItem(paddedNumber, imageUrl, hdUrl, filename) {
    ...
    img.loading = 'lazy';
    img.style.transition = 'transform 0.2s ease'; // Smooth zoom effect
    ...
    item.style.willChange = 'transform'; // Hint for GPU acceleration
    ...
    try {
      ...
      if (err instanceof TypeError) {
        try {
          ...
          console.warn('Fallback also falhou:', url, fallbackErr);
        }
      } else {
        console.warn('Erro ao verificar imagem:', url, err);
      }
    }
    return false;
  }
  ...
  if (window.Fancybox) {
    Fancybox.bind('[data-fancybox="gallery"]', {
      ...
      Carousel: {
        friction: 0.9, // smoother dragging
        Panzoom: {
          decelFriction: 0.9,
          maxScale: 2,
        },
